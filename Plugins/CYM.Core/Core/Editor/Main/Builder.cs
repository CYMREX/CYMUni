...
namespace CYM
{
    public class Builder
    {
        ...
        static void BuildDLCConfigInternal(DLCItem dlcItem)
        {
            ...
            foreach (var bundleData in dlcManifest.Data)
            {
                ...
                foreach (var pathData in bundleData.AssetFullPaths)
                {
                    ...
                    //跳过指定的Bundle资源
                    if (pathData.BuildRuleName == Const.BN_System ||
                        pathData.BuildRuleName == Const.BN_Shared ||
                        pathData.BuildRuleName == Const.BN_Config ||
                        pathData.BuildRuleName == Const.BN_CSharp ||
                        pathData.BuildRuleName == Const.BN_Excel ||
                        pathData.BuildRuleName == Const.BN_Lua ||
                        pathData.BuildRuleName == Const.BN_Text ||
                        pathData.BuildRuleName == Const.BN_Language ||
                        pathData.BuildRuleName == Const.BN_Sprite)
                        continue;
                    if (pathData.SourceBundleName == Const.BN_System ||
                        pathData.SourceBundleName == Const.BN_Shared ||
                        pathData.SourceBundleName == Const.BN_Config ||
                        pathData.SourceBundleName == Const.BN_CSharp ||
                        pathData.SourceBundleName == Const.BN_Excel ||
                        pathData.SourceBundleName == Const.BN_Lua ||
                        pathData.SourceBundleName == Const.BN_Text ||
                        pathData.SourceBundleName == Const.BN_Language ||
                        pathData.SourceBundleName == Const.BN_Sprite)
                        continue;
                    //保证变量名称有效
        ...
        protected static void BuildSharedAssets(DLCItem dlc)
        {
            ...
            foreach (var item in AllDependencies)
            {
                ...
                {
                    //bundle里的资源不包含到共享包
                    if (assetPath.Contains(Const.Dir_Bundles))
                        continue;
                    //resource里的资源不包含到共享包
                    if (assetPath.Contains(Const.Dir_Resources))
                        continue;
        ...
        static bool PreSimulateEditor = false;
        static void OnPreBuild()
        {
            //PreSimulateEditor = BuildConfig.Ins.IsSimulationEditor;
            //BuildConfig.Ins.IsSimulationEditor = true;
        ...
        static void OnPostBuild()
        {
            ...
            #endregion
            //BuildConfig.Ins.IsSimulationEditor = PreSimulateEditor;
        }
        #endregion
    ...
    public class BuildData
    {
        ...
        public virtual void PostBuilder()
        {
            if (BuildConfig.Platform == Platform.Windows64)
            {
                FileUtil.WriteFile(Path.Combine(BuildConfig.DirPath, CopyRight.CopyRightFileName), CopyRight.Notice);
            }
        }
    }
    #endregion
