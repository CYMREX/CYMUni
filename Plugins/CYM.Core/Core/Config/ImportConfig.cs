...
namespace CYM
{
    ...
    public class TextureImportSettings
    {
        ...
        public bool IsContainInDirectoryTag(string path)
        {
            ...
            if (dirRoot == SpriteDirRoot.Bundle && !split.Contains(Const.Dir_Bundles))
            {
                return false;
            }
            if (dirRoot == SpriteDirRoot.Art && !split.Contains(Const.Dir_Art))
            {
                return false;
            }
    ...
    public class AudioImportSettings
    {
        ...
        {
            HashSet<string> split = new HashSet<string>(path.Split('/'));
            if (!split.Contains(Const.Dir_Bundles))
            {
                return false;
            }
            if (!split.Contains(Const.Dir_Art))
            {
                return false;
            }
