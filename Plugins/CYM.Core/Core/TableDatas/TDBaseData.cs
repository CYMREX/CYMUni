    ...
    public class TDBaseData : IBase, ICloneable, ITDBaseData
    {
        ...
        public virtual Sprite GetDisIcon()
        {
            if (Icon == CustomInvalidID)
                return null;
            if (!Icon.IsInv())
                return BaseGlobal.RsIcon.Get(Icon + Const.Suffix_Disable,false);
            return BaseGlobal.RsIcon.Get(TDID + Const.Suffix_Disable,false);
        }
        public Sprite GetSelIcon()
        {
            if (Icon == CustomInvalidID)
                return null;
            if (!Icon.IsInv())
                return BaseGlobal.RsIcon.Get(Icon + Const.Suffix_Sel,false);
            return BaseGlobal.RsIcon.Get(TDID + Const.Suffix_Sel,false);
        }
        // prefab
