...
namespace CYM.DLC
{
    ...
    public class DLCItem
    {
        ...
        public DLCItem(DLCItemConfig config, AssetBundleManifest assetBundleManifest=null)
        {
            ...
            //计算Manifest路径
            if (IsEditorMode) ManifestPath = Path.Combine(AssetsRootPath, Const.Dir_Config);
            else ManifestPath = Path.Combine(TargetPath, Const.Dir_Config);
            //计算Config路径
            if (IsEditorMode) ConfigPath = Path.Combine(AssetsRootPath, Const.Dir_Config);
            else ConfigPath = Path.Combine(TargetPath, Const.Dir_Config);
            //计算语言包路径
            if (IsEditorMode) LanguagePath = Path.Combine(AssetsRootPath, Const.Dir_Language);
            else LanguagePath = Path.Combine(TargetPath, Const.Dir_Language);
            //计算lua路径
            if (IsEditorMode) LuaPath = Path.Combine(AssetsRootPath, Const.Dir_Lua);
            else LuaPath = Path.Combine(TargetPath, Const.Dir_Lua);
            //计算Text路径
            if (IsEditorMode) TextPath = Path.Combine(AssetsRootPath, Const.Dir_TextAssets);
            else TextPath = Path.Combine(TargetPath, Const.Dir_TextAssets);
            //计算CS路径
            if (IsEditorMode) CSPath = Path.Combine(AssetsRootPath, Const.Dir_CSharp);
            else CSPath = Path.Combine(TargetPath, Const.Dir_CSharp);
            //计算Excel路径
            if (IsEditorMode) ExcelPath = Path.Combine(AssetsRootPath, Const.Dir_Excel);
            else ExcelPath = Path.Combine(TargetPath, Const.Dir_Excel);
        ...
        public string GetManifest() => Path.Combine(ManifestPath, Const.STR_DLCManifest + ".json");
        public string GetConfig() => Path.Combine(ConfigPath, Const.STR_DLCItem + ".json");
        ...
        public byte[] GetExcelFile(string name)
        {
            ...
            {
                var asset = LoadRawAsset<TextAsset>(Const.BN_Excel, name);
                if (asset != null)
                {
                    return asset.bytes;
                }
        ...
        public DLCManifest GetManifestFile()
        {
            ...
            {
                var asset = LoadRawAsset<TextAsset>(Const.BN_Config, "DLCManifest");
                if (asset != null)
                {
        ...
        private string CalcAssetRootPath(string dlcName)
        {
            if (IsEditorMode)
            {
                ...
                if (IsInternal)
                {
                    editorDLCPath = Const.RPath_InternalBundle;
                    return editorDLCPath;
                }
                else if (IsNative)
                {
                    editorDLCPath = Const.RPath_Bundles;
                    return editorDLCPath;
                }
                else
                {
                    editorDLCPath = Const.RPath_Dlc;
                    return Path.Combine(editorDLCPath, dlcName);
                }

            }
            else return Path.Combine(Const.Path_StreamingAssets, dlcName);
        }
        #endregion

        #region is
        public bool IsNative => Name == Const.STR_NativeDLC;
        public bool IsInternal => Name == Const.STR_InternalDLC;
        #endregion
    }
